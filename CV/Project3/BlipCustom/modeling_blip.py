...
from transformers.utils import ModelOutput, auto_docstring, logging, torch_int
...
class BlipForQuestionAnswering(BlipPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        return BlipTextVisionModelOutput(
            ...
            hidden_states=vision_outputs.hidden_states,
            attentions=vision_outputs.attentions,
            decoder_cross_attentions=answer_output.cross_attention
        )
    ...
    @torch.no_grad()
    def generate_with_cross_attentions(
        self,
        input_ids: torch.LongTensor,
        pixel_values: torch.FloatTensor,
        attention_mask: Optional[torch.LongTensor] = None,
        ...
        # 2. Encode question
        if isinstance(input_ids, list):
            input_ids = torch.LongTensor(input_ids).to(image_embeds.device)
        else:
        ...
        # 3. Collect cross-attentions (text to image)
        question_image_cross_attentions = question_outputs.cross_attentions

        # 4. Decoder generation
        generated = torch.full((input_ids.size(0), 1), self.decoder_start_token_id, dtype=torch.long, device=image_embeds.device)
        decoder_cross_attentions_per_step = []
        ...
        for step in range(max_length):
            ...
            decoder_cross_attentions_per_step.append(outputs.cross_attentions)

            next_tokens = torch.argmax(next_token_logits, dim=-1, keepdim=True)
            generated = torch.cat([generated, next_tokens], dim=-1)

            if (next_tokens == self.config.text_config.sep_token_id).all():
                break

        # Match return type of .generate() with additional outputs
        return generated, question_image_cross_attentions, decoder_cross_attentions_per_step
